...
class GreenhouseAdapter(ATSAdapter):
    ...
    async def fill(
        ...
        for label in labels:
            ...
            if not value and ask is not None:
                # Unknown field → ask (checks learned qa_log, then pauses for a
                # human via Telegram, then remembers the answer).
                value = await ask(txt)
            if not value:
                continue
...
def _match_answer(plan: ApplicationPlan, label: str) -> str | None:
    ...
    # Anything else is deferred to the orchestrator's `ask` callback, which
    # checks the learned qa_log and, failing that, pauses for a human.
    return None
